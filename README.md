...
## Turbo Pet Mascot & Animated UI Feature Branch

This branch (`feature/turbo-stock-rescue-ui`) contains the interactive animated pet mascot and copilot UI:
- **Turbo the Rescue Hound**: Animated SVG mascot with dynamic tail wagging, floppy ears, blinking eyes, and panting tongue animations.
- **Interactive Pet Care**: 'Pet Me' and 'Give Treat' actions with floating particle feedback (hearts, bones) and synthetic sound effects.
- **Dynamic Mood System**: Turbo responds to operational actions (Zoomies mode, Sniffing patrol, and Suspended Dealer alerts).
- **Retractable UI & Zen Mode**: Instant collapse toggles for focused operational workflows.
